...
function tuiTryParseMarkdownCodeBlock(text2 = "") {
  const tokens = new lib_default().parse(text2, {});
  const result = tokens.filter(({ tag, type }) => tag === "code" && type === "fence").map(({ content }) => content.trim());
  return result.length ? result : [text2];
}
...
var TUI_DOC_DIRECTION_ENABLED = new InjectionToken(ngDevMode ? "TUI_DOC_DIRECTION_ENABLED" : "", { factory: TUI_FALSE_HANDLER });
...
//# sourceMappingURL=@taiga-ui_addon-doc_tokens.js.map
